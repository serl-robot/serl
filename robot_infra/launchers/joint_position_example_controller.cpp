...
void JointPositionExampleController::update(const ros::Time& /*time*/,
  ...
  reset_pose_[0] = 0.001550;
  reset_pose_[1] = -0.225743;
  reset_pose_[2] = -0.045209;
  reset_pose_[3] = -2.687688;
  reset_pose_[4] = -0.082352;
  reset_pose_[5] = 2.533348;
  reset_pose_[6] = 1.014246;
  double delta_angle = M_PI / 16 * (1 - std::cos(M_PI / 5.0 * elapsed_time_.toSec())) * 0.2;
  for (size_t i = 0; i < 7; ++i) {
    if (elapsed_time_.toSec() > 30){
      position_joint_handles_[i].setCommand(reset_pose_[i]); // - delta_angle);
    }
    else {
      position_joint_handles_[i].setCommand( ((elapsed_time_.toSec()) / 30.0) * reset_pose_[i] + ((30 - elapsed_time_.toSec()) / 30.0) * initial_pose_[i]);
    }
  }
}
