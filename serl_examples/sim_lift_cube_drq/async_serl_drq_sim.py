...
import mjenv # import sim franka env
...
def learner(rng, agent: DrQLearner, replay_buffer, replay_iterator, wandb_logger=None, tunnel=None):
    """
...
def main(_):
    ...
    image_keys = [key for key in env.observation_space.spaces.keys() if "state" not in key]
    env = FrameStack(env, 1, stacking_key=image_keys)
    ...
    agent: DrQLearner = globals()[model_cls].create(
        FLAGS.seed, env.observation_space, env.action_space, pixel_keys=image_keys, **kwargs
    )
    import ipdb; ipdb.set_trace()
    # replicate agent across devices
    # need the jnp.array to avoid a bug where device_put doesn't recognize primitives
    agent: DrQLearner = jax.device_put(
